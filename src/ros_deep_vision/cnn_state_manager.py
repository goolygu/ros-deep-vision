...
class CNNStateRequest:
    def __init__(self):
        self.expected_aspect_list=None
        self.aspect_idx_list=None
        self.use_last_observation=False
        self.data_name=None
        self.mode="default"

class CNNListState:
    def __init__(self):
        # list of Dic2 of value
        self.value_dic_list = []
        self.xyz_dic_list = []
        self.xy_dic_list = []
        self.centroid_list = []
...
class CNNStateManager:
    def __init__(self, settings, data_setting_case = "cnn_features", mode = "default", replay_dir = None, replay_folder = None):
        ...
        self.default_load_path = settings.ros_dir + '/current/'
        self.save_path = settings.ros_dir + '/current/'

        # self.data_monster.set_train_path(self.observe_path)

        self.data_collector = DataCollector(self.save_path)
    ...
    def set_mode(self, mode):
        self.mode = mode
        if mode == "replay":
            # self.load_path = self.replay_dir
            self.load_path_list = []
            for folder in self.replay_folder.split(":"):
                self.load_path_list.append(self.replay_dir + "/" + folder + "/")
        else:
            # self.load_path = self.default_load_path
            self.load_path_list = [self.default_load_path]
    ...
    def capture_input(self, save_mask=True):
    ...
    def get_cnn_list_state(self, state_list=None):
        ...
        while (not rospy.is_shutdown()) and (data.img is None or data.mask is None or data.pc is None):
            for load_path in self.load_path_list:
                try:
                    self.data_monster.input_manager.load_img_mask_pc(data, load_path)
                    break
                except IOError:
                    pass
            time.sleep(0.1)
        ...
        cv2.imwrite(self.save_path + data.name + "_" + item_name + "_rgb_crop.png", data.img[:, :, (2,1,0)])
        cv2.waitKey(100)
    ...
    def get_clustered_cnn_list_state(self, req):
        ...
        if self.mode == "replay":
            save_data_name = req.data_name
        elif req.use_last_observation:
            save_data_name = self.last_data_name
        else:
            save_data_name = self.capture_input(save_mask=False)#"current_15-01-2017-20:38:40"#
            self.last_data_name = save_data_name
            time.sleep(0.3)

        resp.name = save_data_name
        ...
        if req.mode == "refine":
            box_min_max_list = [box_min_max_list[aspect_idx] for aspect_idx in req.aspect_idx_list]
            centroid_list = [centroid_list[aspect_idx] for aspect_idx in req.aspect_idx_list]

        resp.centroid_list = centroid_list

        for i, box_min_max in enumerate(box_min_max_list):
            print "handle box", box_min_max
            if req.mode == "refine":
                item_num = req.aspect_idx_list[i]
            ...
            while (not rospy.is_shutdown()) and (data.img is None or data.mask is None or data.pc is None):
                for load_path in self.load_path_list:
                    try:
                        self.data_monster.input_manager.load_img_mask_pc_seg(data, load_path, item_num)
                        break
                    except IOError:
                        pass
                time.sleep(0.1)
            ...
            cv2.imshow("mask_"+item_name, data.mask)
            if self.mode == "replay":
                img_name = self.save_path + data.name + "_" + item_name + "_replay_rgb.png"
            else:
                img_name = self.save_path + data.name + "_" + item_name + "_rgb.png"

            cv2.imwrite(img_name, data.img[:, :, (2,1,0)])
            resp.img_name_list.append(img_name)
            ...
            if req.mode == "default":
                ...
                # if i >= len(req.expected_aspect_list):
                #     break
                expected_dist = state_list_to_dist(req.expected_aspect_list[i].state_list)
                # print "expected_dist", expected_dist.filter_tree
                filter_xyz_dict, filter_xy_dict, value_dict = self.data_monster.get_state(expected_dist, data)
            elif req.mode == "refine_action":
                expected_dist = state_list_to_dist(req.expected_aspect_list[0].state_list)
                filter_xyz_dict, filter_xy_dict, value_dict = self.data_monster.get_state(expected_dist, data)
            else:
                print "error, no such request mode"

            print "show feature"
            self.data_monster.show_feature(filter_xyz_dict, item_name)

            resp.value_dic_list.append(value_dict)
            resp.xyz_dic_list.append(filter_xyz_dict)
            resp.xy_dic_list.append(filter_xy_dict)
            # state_list, pose_list = to_state_pose_list(value_dict, filter_xyz_dict)
